...
resource "yandex_compute_instance" "builder" {
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo docker run --rm -v /tmp:/tmp cr.yandex/crpirf1t243rd4chrqfd/maven /bin/bash -c \"git clone 'https://github.com/wipefree/warhello.git' /tmp/prj && mvn -f /tmp/prj/pom.xml clean package -DskipTests\"",
      "cd /tmp/prj",
  ...
  provisioner "local-exec" {
    command = "touch .builder_end"
  }

}

# ssh -o StrictHostKeyChecking=no ubuntu@158.160.126.98
# yc iam key create --service-account-name andres-srv -o ~/.ssh/key.json
